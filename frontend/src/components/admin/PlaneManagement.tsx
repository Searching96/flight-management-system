...
const PlaneManagement: React.FC<{
  ...
  const filteredPlanes = planes.filter((plane) => {
    const matchesSearch =
      plane.planeCode.toLowerCase().includes(searchTerm.toLowerCase()) ||
      plane.planeType.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesType =
      !filterType ||
      plane.planeType.toLowerCase().includes(filterType.toLowerCase());
    return matchesSearch && matchesType;
  });

  // Get unique plane types for filter - combine existing fleet types with predefined options
  const existingPlaneTypes = [
    ...new Set(planes.map((plane) => plane.planeType)),
  ];
  const predefinedTypes = aircraftTypeOptions.map((option) => option.value);
  const allPlaneTypes = [
    ...new Set([...existingPlaneTypes, ...predefinedTypes]),
  ].sort();
    ...
    <Container fluid className="py-4">
                  ...
                  <Row className="text-center">
                    ...
                    <Col>
                      <Badge bg="info" className="p-2">
                        ...
                        <strong>
                          {planes.reduce(
                            (sum, plane) => sum + plane.seatQuantity,
                            0
                          )}
                        </strong>
                      </Badge>
      ...
      <Modal show={showForm} onHide={handleCancel} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>
            {editingPlane ? "Chỉnh sửa máy bay" : "Thêm máy bay mới"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
