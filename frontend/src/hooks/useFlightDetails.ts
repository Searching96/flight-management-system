...
export function useFlightDetails() {
    ...
    const loadFlightDetails = useCallback(async (flightId: number): Promise<boolean> => {
        try {
            setLoading(true);
            ...
            const detailsWithIndex = details.map((detail, idx) => ({
                ...detail,
            ...
            setError('Không thể tải thông tin sân bay trung gian: ' + (err.message || 'Lỗi không xác định'));
            return false;
        } finally {
            setLoading(false);
        }
    ...
    const saveFlightDetails = useCallback(async (flightId: number): Promise<boolean> => {
        try {
            ...
            console.log(`Preparing to save ${flightDetails.length} flight details for flight ID:`, flightId);

            // Filter out incomplete details before saving and remove UI-only fields
            const validDetailsToSave = flightDetails
                .filter(detail => detail.mediumAirportId !== 0 && detail.arrivalTime)
            ...
            try {
                ...
                await Promise.all([
                    ...
                    ...existingDetails
                        .filter(existingDetail => 
                            !validDetailsToSave.some(newDetail => 
                                newDetail.mediumAirportId === existingDetail.mediumAirportId))
                        .map(detail => {
                    ...
                    ...validDetailsToSave
                        ...
                        .map(detail => {
                            ...
                            const existingDetail = existingDetails.find(ed => 
                                ed.mediumAirportId === detail.mediumAirportId);
            ...
            setError('Không thể lưu thông tin sân bay trung gian: ' + (err.message || 'Lỗi không xác định'));
            return false;
        }
    }, [flightDetails]);
    ...
    const updateFlightDetail = useCallback((index: number, field: string, value: any) => {
        setFlightDetails(prevDetails => {
